...
class AnalisadorSemantico:
    ...
    def _r07(self):
        for c in self.h.componentes:
            med = _f(c.media)
            if c.situacao == 'APR' and med is not None and med < 7.0:
                self.erros.append(f'[Semântico] R07: {c.cod_turma} ({c.periodo}) APR mas média {med:.1f} < 7.0.')
    ...
    def _r08(self):
        for c in self.h.componentes:
            if c.situacao in ('REP', 'REPMF'):
                med = _f(c.media)
                if med is not None and med >= 7.0:
                    self.erros.append(f'[Semântico] R08: {c.cod_turma} ({c.periodo}) {c.situacao} mas média {med:.1f} ≥ 7.0.')
